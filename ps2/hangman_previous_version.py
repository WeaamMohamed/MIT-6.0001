...
Available letters: abcdefghijklmnopqrstuvwxyz''')
    ...
    while(True):
        ...
        letter_input, warnings = get_user_input(warnings)
    ...
    if(is_word_guessed(secret_word, letters_guessed)):
        print(f"\nCongratulateions, your guessed word is {secret_word}!")
        
    else:
        print(f"\nsorry you have failed, the secret word is {secret_word}")
...
def get_user_input(warnings):
    ...
    if len(letter) != 1:
        warnings -=1
        print(f"letter must be one character try again... You have {warnings} warnings left ") 
