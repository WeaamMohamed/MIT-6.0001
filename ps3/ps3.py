...
def get_word_score(word, n):
    ...
    if second_component <= 0: second_component = 1
...
def update_hand(hand, word):
    ...
    for char in word.lower():
        if char in updated_hand:
            updated_hand[char] -=1
            if updated_hand[char] < 1: updated_hand.pop(char)
...
#todo: try to refactor this ,, can we use recursion??
...
def is_valid_word(word, hand, word_list):
    ...
    if word_copy.__contains__("*"):
        for vol in VOWELS: 
            word_copy = word.lower().replace('*', vol)
            if word_copy in word_list:
                word_copy = word_copy.replace("*", vol)
    ...
    #print("last True return ")
    return True  # TO DO... Remove this line when you implement this function
    ...
    while(calculate_handlen(hand)):
        ...
        # Otherwise (the input is not two exclamation points):
            ...
            if is_valid_word(user_input, hand, word_list):
                ...
                print(f"({user_input}) earned { get_word_score(user_input, n)} points. Total: {total_score} points")
...
def substitute_hand(hand, letter):
    ...
    hand_copy = hand.copy()
    new_letter = letter
    if letter in CONSONANTS: rand = CONSONANTS 
    else: rand = VOWELS
    
    while new_letter in hand_copy.keys():
        new_letter = random.choice(rand)
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    total_hands_num = int(input('Enter total number of hands: '))
    total_scores = list()
    for i in range(total_hands_num):
    ...
    print(f"Total score over all hands: {sum(total_scores)}") 
